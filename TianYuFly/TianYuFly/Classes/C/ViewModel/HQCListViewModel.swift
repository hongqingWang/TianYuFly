...
class HQCListViewModel {
    
    lazy var cList = [HQCViewModel]()
    ...
    func loadData(pullup: Bool, completion: @escaping (_ isSuccess: Bool, _ shouldRefresh: Bool) -> ()) {
        ...
        let since_id = pullup ? 0 : (cList.first?.model.id ?? 0)
        // 上拉刷新,取出数组的最后一条微博`id`
        let max_id = !pullup ? 0 : (cList.last?.model.id ?? 0)
        
        HQNetWorkManager.shared.cList(since_id: since_id, max_id: max_id) { (list, isSuccess) in
            ...
            for dict in list ?? [] {
                ...
                // 使用`普通模型`创建`视图模型`
                let viewModel = HQCViewModel(model: model)
