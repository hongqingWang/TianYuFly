...
class HQAViewModel {
    ...
    /// 视图模型数组懒加载
    lazy var viewModelArr = [HQAaViewModel]()
    ...
    func loadStatus(pullup: Bool, completion: @escaping (_ isSuccess: Bool, _ shouldRefresh: Bool)->()) {
        ...
        let since_id = pullup ? 0 : (viewModelArr.first?.model.id ?? 0)
        // 上拉刷新,取出数组的最后一条微博`id`
        let max_id = !pullup ? 0 : (viewModelArr.last?.model.id ?? 0)
        ...
        HQNetWorkManager.shared.statusList(since_id: since_id, max_id: max_id) { (list, isSuccess) in
            ...
            for dict in list ?? [] {
                ...
                // 使用`普通模型`创建`视图模型`
                let viewModel = HQAaViewModel(model: model)
            ...
            if pullup {
                // 上拉刷新结束后,将数据拼接在数组的末尾
                self.viewModelArr += array
                
            } else {
                // 下拉刷新结束后,将数据拼接在数组的最前面
                self.viewModelArr = array + self.viewModelArr
            }
