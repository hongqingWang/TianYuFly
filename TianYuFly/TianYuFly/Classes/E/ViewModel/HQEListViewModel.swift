...
class HQEListViewModel {
    
    lazy var eList = [HQEViewModel]()
    ...
    func loadData(pullup: Bool, completion: @escaping (_ isSuccess: Bool, _ shouldRefresh: Bool) -> ()) {
        ...
        let since_id = pullup ? 0 : (eList.first?.model.id ?? 0)
        // 上拉刷新,取出数组的最后一条微博`id`
        let max_id = !pullup ? 0 : (eList.last?.model.id ?? 0)
        
        HQNetWorkManager.shared.eList(since_id: since_id, max_id: max_id) { (list, isSuccess) in
            ...
            for dict in list ?? [] {
                ...
                // 使用`普通模型`创建`视图模型`
                let viewModel = HQEViewModel(model: model)
